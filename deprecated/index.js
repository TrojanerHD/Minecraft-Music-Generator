...
ipcMain.on('add-path', (event, arg) => {
  ...
  if (fs.existsSync('paths.json'))
    content = JSON.parse(fs.readFileSync('paths.json').toString());
  content.push(arg);
...
ipcMain.on('save', (event, arg) => {
  ...
  /* fs.writeFile(
    path.join(arg['path'], 'save.json'),
    JSON.stringify(arg['result']),
    err => {
      if (err) console.error(err);
    }
  ); */
...
ipcMain.on('get-saved-content', (event, arg, oldResult) => {
  ...
  event.returnValue =
    savedContent !== '' ? JSON.parse(savedContent) : oldResult;
});
